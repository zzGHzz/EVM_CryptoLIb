...
contract Secp256k1 {
    ...
    // checked
    function decompress(uint8 yBit, uint x) public pure returns (uint[2] memory P) {
        uint p = pp;
    ...
    // outData: Rx, Ry, Rz
    function _addMixed(uint[3] memory P, uint[2] memory Q) public pure returns (uint[3] memory R) {
        if(P[2] == 0)
            return [Q[0], Q[1], 1];
        if(Q[1] == 0)
            return P;
    ...
    function _addMixedM(uint[3] memory P, uint[2] memory Q) public pure {
        if(P[1] == 0) {
            P[0] = Q[0];
            P[1] = Q[1];
            P[2] = 1;
    ...
    function _doubleM(uint[3] memory P) public pure {
        uint p = pp;
        if (P[2] == 0)
            return;
        uint Px = P[0];
